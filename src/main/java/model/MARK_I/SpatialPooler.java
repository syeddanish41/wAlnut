...
public class SpatialPooler extends Pooler {
    ...
    private Set<model.MARK_II.ColumnPosition> activeColumnPositions;
    ...
    public SpatialPooler(Region region) {
        ...
        this.activeColumnPositions = new HashSet<model.MARK_II.ColumnPosition>();
    }
     ...
     */
    public Set<model.MARK_II.ColumnPosition> getActiveColumnPositions() {
    ...
    void computeActiveColumnsOfRegion() {
        ...
        for (int row = 0; row < columns.length; row++) {
            for (int column = 0; column < columns[0].length; column++) {
                ...
                List<model.MARK_II.ColumnPosition> neighborColumnPositions = new ArrayList<model.MARK_II.ColumnPosition>();
                neighborColumnPositions = columns[row][column].getNeighborColumns();
                List<Column> neighborColumns = new ArrayList<Column>();
                for (model.MARK_II.ColumnPosition columnPosition : neighborColumnPositions) {
                    neighborColumns
                ...
                if (columns[row][column].getOverlapScore() > 0
                    ...
                    this.activeColumnPositions.add(new model.MARK_II.ColumnPosition(row, column));
                }
            }
        }
    }
        ...
        for (int row = 0; row < columns.length; row++) {
            for (int column = 0; column < columns[0].length; column++) {
                if (columns[row][column].getActiveState()) {
                    ...
                    List<model.MARK_II.ColumnPosition> neighborColumnPositions = columns[row][column]
                            .getNeighborColumns();
